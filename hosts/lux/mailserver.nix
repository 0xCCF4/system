{ lib, self, config, luxAddr6For, luxPublicNetwork6, mailserver, specialArgs, ... }: with lib; {
  config.containers.mailserver =
    ...
    in
    {
      autoStart = false;
      privateNetwork = true;
      # IPv6-only: a real address out of lux's routed /64 (no NAT, no
      # forwardPorts - see hosts/lux/default.nix's nftables allowlist for the
      # ports actually reachable from the internet). Both addresses are
      # hash-derived (see hosts/lux/net.nix) rather than hand-numbered, so the
      # real /64 prefix isn't reconstructable from public repo content, and
      # small-integer suffixes aren't enumerable by scanners.
