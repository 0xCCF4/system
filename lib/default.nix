...
let
  ...
  ipv6 = import ./ipv6.nix {
    inherit lib;
    netLib = inputs.noxa.inputs.nix-net-lib.lib;
  };
in
{
  flake = {
    lib = rec {
      ...
      evalMissingOption =
        ...
        evalMissingOption' false config option default;
      evalMissingOption' =
        silence: config: option: default:
        let
          ...
          optionAfterMine =
            if optionStartsWithMine then strings.concatStringsSep "." (tail options) else option;
        ...
        (attrsets.attrByPath options
          (
            ...
            (optionalTrace "${bold + fgMagenta + underline}Warning${noUnderline}: ${noBold + fgYellow} nixos module option ${fgCyan + optionBeforeMine + italic + optionAfterMine + fgYellow + noItalic} module not found! Did you not import the module in your NixOS settings?" default)
          )
          config);
