...
let
  ...
  modulesExternal = removeAttrs (inputs.noxa.lib.nixDirectoryToAttr' ../external/private/users) [
    "default"
  ];
  ...
  uidMappingAll = recursiveUpdate uidMapping (
    if (pathExists ../external/private/users/_mapping.json) then uidExternalMapping else { }
  );
  ...
  allUsers = listToAttrs (
    map
      (
        ...
        nameValuePair name {
          description =
            usersExternal.${name}.description or users.${name}.description or "No description provided.";
          authorizedKeys =
            (usersExternal.${name}.authorizedKeys or [ ]) ++ (users.${name}.authorizedKeys or [ ]);
          ...
          os =
            ...
            let
              modules = [
                ((users.${name}.os or ({ ... }: { })) inputs)
              ]
              ++ [ ((usersExternal.${name}.os or ({ ... }: { })) inputs) ];
            in
