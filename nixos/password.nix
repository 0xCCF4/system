    ...
    let
      spec = mapAttrsToList
        (_: u: ''
          ...
          cat ${escapeShellArg u.passwordFileOverride} | ${getExe pkgs.mkpasswd} -s -m sha512crypt > ${escapeShellArg u.hashedPasswordFile}
        '')
        (filterAttrs (_: u: u.passwordFileOverride != null) config.users.users);
    in
    mkIf (!config.age.rekey.initialRollout && (length spec) > 0) {
