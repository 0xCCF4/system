...
with builtins;
{
  ...
  options.mine.unfree = with types; {
  ...
  config =
    ...
    in
    {
      ...
      mine.unfree.allowList = mkIf (cfg.allowUserWhitelist) (
        mkMerge (
          map (user: config.home-manager.users.${user}.home.mine.unfree.allowList) (config.mine.users or [ ])
        )
      );
    };
}
